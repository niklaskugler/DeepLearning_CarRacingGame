...
DenseNet = modelDefinition.DenseNet(in_channels=2, layer_size=[8, 8, 8], growth_rate=12, additional_neurons=8).to(device=device)  #initialize DenseNet - Adapt Parameters!!!
...
normalized_data = dataLoading.SteeringCommands(csv_file=dataLoading.csv_path, root_dir=r"F:\Meine Programme\OpenAI_RaceGame\Data_Scripts\processed_training_data_saved", transform=transform)    #create new normalized training set
...
nr_epochs = 10   #hyperparameter - determnies how many times the whole training set gets looped through the Neural Network
