...
class Agent:
    def __init__(self, mean_r=1.1384, mean_g=194.1198, std_r=16.9998,
        ...
        self.DenseNet.load_state_dict(torch.load(
            r"C:\Users\Jakob Häringer\Documents\01 Studium\08 6. Semester\06 Software-Projekt\Trainingsession_Eigene\2\trained_densenet_epoch_70.pth",
    ...
    def driver(self, state):
        ...
        commands[0] = 0
        if output[0] > 0.8:  # if left key activated
            commands[0] -= 1.0
        if output[1] > 0.8:  # if right key activated
            commands[0] += 1.0
        if output[2] > 0.95:  # if up key activated
            commands[1] = 1.0
        if output[3] > 0.45:  # if down key activated
            commands[2] = 0.8
