...
class DenseNet(nn.Module):
    def __init__(self, in_channels=3, layer_size=None, growth_rate=12,
        ...
        # Global Average Pooling -> Compresses all channels of size 12x13 to size 1x1 (for input neurons)
    ...
    def forward(self, x, list_additional_inputs=[1, 0.5]):
        # x = Tensor of Image (96*85*in_channels -> specified in constructor of DenseNet), a = List of other Inputs)
        print("Size before First Convolution:", x.shape)
        ...
        list_additional_inputs = torch.unsqueeze(list_additional_inputs, 0)
        # unsqueeze tensor, so it can be concatenated with tensor x
        x = torch.cat((x, list_additional_inputs), 1)
        # concatenate tensor x and list_additional_inputs -> new tensor of size([1,1026])
