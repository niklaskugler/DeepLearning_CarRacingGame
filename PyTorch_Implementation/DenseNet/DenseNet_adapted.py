...
class DenseNet(nn.Module):
    def __init__(self, in_channels=3, layer_size=[6,6,7,8], growth_rate=12, additional_neurons = 2):  #in_channels describes picture channels (3 for rgb)  #layer_size must be list of 4 Elements
        ...
        print("In Fully Connected: " ,self.in_channels)
    ...
    def forward(self, x, a = [1, 0.5]):   #x = Tensor of Image (96*85*in_channels -> specified in constructor of DenseNet), a = List of other Inputs
        ...
        x = torch.flatten(x, 1)
        a = torch.Tensor(a)
        x = torch.squeeze(x, 0)     #remove unnessecary dimension from image tensor in order to concatenate it with tensor a
        if(len(a.size()) > 1):  #checks if a is passed as batch or single value
            ...
            x = torch.cat((x,a),0)      #concatenate tensor x and a along dimension 0
...
image=glob.glob("F:\Meine Programme\OpenAI_RaceGame\Training_Data\processed_image102.png")
img = Image.open(image[0])
img = transforms.ToTensor()(img).unsqueeze_(0)  #size(1,3,96,84)
print(img.size())
ipt = [1, 0.5]  #true speed, steering angle
ipt = torch.Tensor(ipt)
with torch.no_grad():
    CNN.eval()  #activate evaluation mode
print("Output of CNN:", CNN(img, ipt))    #pass smaple image to CNN and print output
print("Amount of learnable Parameters: ", count_parameters(CNN))

"""Code to save & Load Parameters:
